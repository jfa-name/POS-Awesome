        ...
        >
          ...
          >
            ...
            <v-date-picker
              ...
              @update:model-value="deliverynote_posting_date = false"
            >
            </v-date-picker>
          </v-menu>
        </v-col>
      ...
      >
        ...
        >
          ...
          <template v-slot:expanded-row="{ item }">
            <td colspan="100%" class="ma-0 pa-0">
              ...
              <v-row class="ma-0 pa-0">
                ...
                >
                  ...
                  >
                    <template v-slot:item="{ item: batchItem, props: batchProps }">
                      <v-list-item v-bind="batchProps" :title="undefined">
                        <v-list-item-title
                          v-html="batchItem.raw.batch_no"
                        ></v-list-item-title>
                        <v-list-item-subtitle
                          v-html="
                            `Available QTY  '${batchItem.raw.batch_qty}' - Expiry Date ${batchItem.raw.expiry_date}`
                          "
                        ></v-list-item-subtitle>
                      </v-list-item>
                    </template>
...
export default {
  ...
  data() {
    return {
      ...
      deliverynoteTypes: ["Delivery Note", "DOrder"],
  ...
  methods: {
    ...
    get_deliverynote_doc() {
      ...
      doc.payments = this.deliverynote_doc.payments || [];
      doc.taxes = [];
    ...
    validate() {
      ...
      this.items.forEach((item) => {
        ...
        if (this.deliverynote_doc.is_return) {
          if (this.subtotal >= 0) {
            evntBus.$emit("show_mesage", {
              text: __(`Return Delivery Note Total Not Correct`),
              color: "error",
            });
            value = false;
            return value;
          ...
          if (this.subtotal * -1 > this.return_doc.total) {
            evntBus.$emit("show_mesage", {
              text: __(`Return Delivery Note Total should not be higher than {0}`, [
          ...
          this.items.forEach((item) => {
            ...
            if (!return_item) {
              evntBus.$emit("show_mesage", {
                text: __(
                  `The item {0} cannot be returned because it is not in the delivery note {1}`,
    ...
    print_draft_deliverynote() {
      if (!this.pos_profile.posa_allow_print_draft_deliverynotes) {
        evntBus.$emit("show_mesage", {
          text: __(`You are not allowed to print draft delivery notes`),
          color: "error",
        });
        return;
      }
    ...
    deliveryChargesFilter(item, queryText) {
  ...
  created() {
    evntBus.$on("register_pos_profile", (data) => {
      ...
      this.deliverynoteType = this.pos_profile.posa_default_sales_order
        ? "DOrder"
        : "Delivery Note";
    });
  ...
  beforeUnmount() {
    ...
    evntBus.$off("new_deliverynote");
    evntBus.$off("load_deliverynote");
    ...
    evntBus.$off("load_return_deliverynote");
    evntBus.$off("set_new_line");
  ...
  watch: {
    ...
    expanded(data_value) {
      ...
      // Resolve keys back to item objects from this.items
      const resolved = data_value
        .map((key) =>
      ...
      this.update_items_details(resolved);
      this.update_item_detail(resolved[0]);
    },
    discount_percentage_offer_name() {
